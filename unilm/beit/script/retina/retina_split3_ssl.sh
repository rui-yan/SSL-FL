...
LR='1.5e-3'
BATCH_SIZE=64
OUTPUT_PATH="/data/yan/SSL-FL/fedavg_model_ckpt/${DATASET}_pretrained_beit_base/pretrained_epoch${EPOCHS}_${SPLIT_TYPE}_lr${LR}_bs${BATCH_SIZE}"

# CUDA_VISIBLE_DEVICES=2 python run_beit_pretraining_FedAvg.py \
#         --data_path ${DATA_PATH} \
#         --data_set ${DATASET} \
...
# LR='5e-3'
FT_EPOCHS=100
OUTPUT_PATH_FT="/data/yan/SSL-FL/fedavg_model_ckpt/${DATASET}_pretrained_beit_base/pretrained_epoch${EPOCHS}_lr${LR}_bs${BATCH_SIZE}/finetune_${DATASET}_epoch${FT_EPOCHS}_${SPLIT_TYPE}_lr${LR}_bs${BATCH_SIZE}"

CUDA_VISIBLE_DEVICES=2 python run_class_finetuning_FedAvg.py \
     --data_path ${DATA_PATH} \
     --data_set ${DATASET} \
     --finetune ${CKPT_PATH} \
     ...
     --E_epoch 1 --max_communication_rounds ${FT_EPOCHS} --num_local_clients -1 
