...
from util.rel_pos_bias import relative_position_bias
...
def valid(args, model, data_loader):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.no_grad():
            ...
            y_preds = torch.sigmoid(output)
            row_sums = torch.sum(y_preds, 1) # normalization 
            row_sums = row_sums.unsqueeze(1)
            row_sums = torch.repeat_interleave(row_sums, args.nb_classes, dim=1)
            y_preds = torch.div(y_preds, row_sums ) # these should be histograms
            all_predictions.extend(y_preds.cpu().detach().numpy().tolist())

        acc1, _ = accuracy(output, target, topk=(1, 2))
        auc = roc_auc_score(all_targets, all_predictions, multi_class='ovr', average='weighted')
        batch_size = images.shape[0]
        
        metric_logger.update(loss=loss.item())
        metric_logger.meters['acc1'].update(acc1.item(), n=batch_size)
        metric_logger.meters['auc'].update(auc*100, n=batch_size)
    
    # gather the stats from all processes
    # metric_logger.synchronize_between_processes()
    print('* Acc@1 {top1.global_avg:.3f} AUC {auc.global_avg:.3f} loss {losses.global_avg:.3f}'
          .format(top1=metric_logger.acc1, auc=metric_logger.auc, losses=metric_logger.loss))
